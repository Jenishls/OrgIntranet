...
public class DBUserTableT
{
    ...
    public DataTable GetCertificate(string username)
    {
        ...
        DataTable dt = DAO.GetTable(param, @"
                ...
                SELECT UserId,examid,nocans,sid,stdate AS SDate, enddate AS EDate 
                INTO #Temp1
    ...
    public DataTable GetCertificateByDate(string username, int examDate)
    {
        SqlParameter[] param = new SqlParameter[]
        {
            new SqlParameter("@i",username),
            new SqlParameter("@e",examDate),
        };
        DataTable dt = DAO.GetTable(param, @"
                SELECT UserId,U.BranchCode,FullName,B.BranchName 
                INTO #Temp
                ...
                SELECT UserId,examid,nocans,sid,stdate AS SDate, enddate AS EDate 
                INTO #Temp1
                FROM dbo.OTexams WHERE UserId=@i AND examid = @e
				ORDER BY enddate DESC
